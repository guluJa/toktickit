...
const ticket = { id: 1, ticketNumber: "TKT-QUEUE-1", summary: "Laptop issue", category: { id: 1, name: "Hardware" }, relatedSystem: { id: 1, name: "Campus Wi-Fi" }, requestedPriority: "HIGH" as const, itPriority: "MEDIUM" as const, currentStatus: "NEW" as const, owner: { id: 10, name: "Staff", email: "staff@example.test", role: "IT_STAFF" as const, isActive: true, mustChangePassword: false }, createdAt: "2026-09-01T00:00:00Z", updatedAt: "2026-09-02T00:00:00Z" };
...
describe("Staff Ticket Queue", () => {
  it("shows loading then desktop table and responsive mobile cards", async () => {
    ...
    expect(screen.getByRole("table")).toBeInTheDocument(); expect(screen.getAllByText("Laptop issue").length).toBeGreaterThan(0); expect(screen.getAllByText(/Staff \(staff@example.test\)/).length).toBeGreaterThan(0);
  });
